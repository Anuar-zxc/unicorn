...
const platforms = ["Kaspi", "Wildberries", "OZON", "Instagram / Other"];
const revenues = ["Under 1M ₸", "1–5M ₸", "5–15M ₸", "15M+ ₸"];
const registrations = ["Yes", "No", "Not sure"];
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-5 md:p-6">
        ...
        <h1 className="mt-5 font-display text-3xl font-semibold">Marketplace Seller Tax Check</h1>
        <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
          Get a practical Kazakhstan tax checklist based on how and where you sell.
        </p>

        <ChoiceGroup label="What platform do you sell on?" values={platforms} value={platform} onChange={setPlatform} />
        <ChoiceGroup label="Monthly revenue range" values={revenues} value={revenueRange} onChange={setRevenueRange} />
        <ChoiceGroup label="Are you registered as ИП?" values={registrations} value={isRegisteredIP} onChange={setIsRegisteredIP} />

        <label className="mt-6 block text-sm font-medium">What do you sell?</label>
        <input
          value={productType}
          onChange={(event) => setProductType(event.target.value)}
          placeholder="Example: clothes, cosmetics, electronics, digital services"
        ...
        <Button className="mt-5 w-full" onClick={run} disabled={loading}>
          ...
          {loading ? "Checking current rules…" : "Create my tax checklist"}
        </Button>
      </section>

      <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-5 md:p-6">
        <p className="text-xs font-semibold uppercase tracking-[.16em] text-[var(--text-muted)]">Personalized report</p>
        <h2 className="mt-2 font-display text-2xl font-semibold">Your likely obligations and next steps</h2>
        <div className="mt-5 min-h-[560px] rounded-xl border border-[var(--border)] bg-[var(--bg-page)] p-5">
          {output ? (
            ...
            <div className="grid min-h-[510px] place-items-center text-center text-sm text-[var(--text-muted)]">
              Complete the form to generate a practical checklist with trusted-source references.
            </div>
          )}
        </div>
      </section>
